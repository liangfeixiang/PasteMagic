...
const AutoCipherTool = ({ content = '' }) => {
  ...
  return (
    <div className="w-full border rounded p-4 space-y-4">
      <h3 className="text-lg font-bold">🔍 自动解密工具</h3>
      ...
      {successfulResults.length > 0 && (
        <div className="space-y-4">
          <div className="text-sm text-green-600 bg-green-50 p-2 rounded">
            🔓 解密成功结果 ({successfulResults.length} 个)
          </div>
            ...
            {successfulResults.map((result, index) => (
              <div key={index} className="border rounded-lg p-4 bg-green-50 border-green-200">
                ...
                <div className="space-y-2">
                  <div className="text-sm font-medium text-green-700">解密成功</div>
      ...
      <div className="border rounded p-3 bg-gray-50">
        <h4 className="font-medium text-sm mb-2 text-gray-700">使用说明</h4>
        <div className="space-y-1 text-sm text-gray-600">
          <div>• 系统将使用所有已保存的加密配置依次尝试解密</div>
          <div>• 只显示成功解密的结果</div>
          <div>• 成功结果以绿色主题突出显示</div>
          <div className="mt-2 pt-2 border-t text-xs">
            <strong>提示:</strong> 请确保已在密钥配置管理中添加了正确的加密配置
          </div>
        </div>
      </div>
    </div>
